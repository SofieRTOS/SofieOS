/*
 * lfs utility functions
 *
 * Copyright (c) 2017, Arm Limited. All rights reserved.
 * SPDX-License-Identifier: BSD-3-Clause
 */
#ifndef LFS_UTIL_H
#define LFS_UTIL_H

// Users can override lfs_util.h with their own configuration by defining
// LFS_CONFIG as a header file to include (-DLFS_CONFIG=lfs_config.h).
//
// If LFS_CONFIG is used, none of the default utils will be emitted and must be
// provided by the config file. To start, I would suggest copying lfs_util.h
// and modifying as needed.
#ifdef LFS_CONFIG
#define LFS_STRINGIZE(x) LFS_STRINGIZE2(x)
#define LFS_STRINGIZE2(x) #x
#include LFS_STRINGIZE(LFS_CONFIG)
#else

// System includes
#include <stdint.h>
#include <stdbool.h>
#include <string.h>
#include <inttypes.h>
#include <sofie.h>

#ifndef LFS_NO_MALLOC
#include <stdlib.h>
#endif
#ifndef LFS_NO_ASSERT
#include <assert.h>
#endif
#if !defined(LFS_NO_DEBUG) || \
        !defined(LFS_NO_WARN) || \
        !defined(LFS_NO_ERROR) || \
        defined(LFS_YES_TRACE)
#include <stdio.h>
#endif

#ifdef __cplusplus
extern "C"
{
#endif


// Macros, may be replaced by system specific wrappers. Arguments to these
// macros must not have side-effects as the macros can be removed for a smaller
// code footprint

// Logging functions
#ifndef LFS_TRACE
#ifdef LFS_YES_TRACE
#define LFS_TRACE_(fmt, ...) \
    printf("%s:%d:trace: " fmt "%s\n", __FILE__, __LINE__, __VA_ARGS__)
#define LFS_TRACE(...) LFS_TRACE_(__VA_ARGS__, "")
#else
#define LFS_TRACE(...)
#endif
#endif

#ifndef LFS_DEBUG
#ifndef LFS_NO_DEBUG
#define LFS_DEBUG_(fmt, ...) \
    printf("%s:%d:debug: " fmt "%s\n", __FILE__, __LINE__, __VA_ARGS__)
#define LFS_DEBUG(...) LFS_DEBUG_(__VA_ARGS__, "")
#else
#define LFS_DEBUG(...)
#endif
#endif

#ifndef LFS_WARN
#ifndef LFS_NO_WARN
#define LFS_WARN_(fmt, ...) \
    printf("%s:%d:warn: " fmt "%s\n", __FILE__, __LINE__, __VA_ARGS__)
#define LFS_WARN(...) LFS_WARN_(__VA_ARGS__, "")
#else
#define LFS_WARN(...)
#endif
#endif

#ifndef LFS_ERROR
#ifndef LFS_NO_ERROR
#define LFS_ERROR_(fmt, ...) \
    printf("%s:%d:error: " fmt "%s\n", __FILE__, __LINE__, __VA_ARGS__)
#define LFS_ERROR(...) LFS_ERROR_(__VA_ARGS__, "")
#else
#define LFS_ERROR(...)
#endif
#endif

// Runtime assertions
#ifndef LFS_ASSERT
#ifndef LFS_NO_ASSERT
#define LFS_ASSERT(test) uassert(test)
#else
#define LFS_ASSERT(test)
#endif
#endif


// Builtin functions, these may be replaced by more efficient
// toolchain-specific implementations. LFS_NO_INTRINSICS falls back to a more
// expensive basic C implementation for debugging purposes

// Min/max functions for unsigned 32-bit numbers
static uint32_t lfs_max(uint32_t a, uint32_t b) {
	sofie_set_protected_stack();
	uint32_t ret = (a > b) ? a : b;
	sofie_restore_protected_stack();
	return ret;
}

static uint32_t lfs_min(uint32_t a, uint32_t b) {
	sofie_set_protected_stack();
	uint32_t ret = (a < b) ? a : b;
	sofie_restore_protected_stack();
	return ret;
}

// Align to nearest multiple of a size
static uint32_t lfs_aligndown(uint32_t a, uint32_t alignment) {
	sofie_set_protected_stack();
    uint32_t ret = a - (a % alignment);
    sofie_restore_protected_stack();
    return ret;
}

static uint32_t lfs_alignup(uint32_t a, uint32_t alignment) {
	sofie_set_protected_stack();
	uint32_t ret = lfs_aligndown(a + alignment-1, alignment);
	sofie_restore_protected_stack();
	return ret;
}

// Find the smallest power of 2 greater than or equal to a
static uint32_t lfs_npw2(uint32_t a) {
	sofie_set_protected_stack();
#if !defined(LFS_NO_INTRINSICS) && (defined(__GNUC__) || defined(__CC_ARM))
	uint32_t ret = 32 - __builtin_clz(a-1);
	sofie_restore_protected_stack();
	return ret;
#else
    uint32_t r = 0;
    uint32_t s;
    a -= 1;
    s = (a > 0xffff) << 4; a >>= s; r |= s;
    s = (a > 0xff  ) << 3; a >>= s; r |= s;
    s = (a > 0xf   ) << 2; a >>= s; r |= s;
    s = (a > 0x3   ) << 1; a >>= s; r |= s;
    uint32_t ret = (r | (a >> 1)) + 1;
    sofie_restore_protected_stack();
    return ret;
#endif
}

// Count the number of trailing binary zeros in a
// lfs_ctz(0) may be undefined
static uint32_t lfs_ctz(uint32_t a) {
	sofie_set_protected_stack();
#if !defined(LFS_NO_INTRINSICS) && defined(__GNUC__)
	uint32_t ret = __builtin_ctz(a);
	sofie_restore_protected_stack();
	return ret;
#else
	uint32_t ret = lfs_npw2((a & -a) + 1) - 1;
	sofie_restore_protected_stack();
	return ret;
#endif
}

// Count the number of binary ones in a
static uint32_t lfs_popc(uint32_t a) {
	sofie_restore_protected_stack();
#if !defined(LFS_NO_INTRINSICS) && (defined(__GNUC__) || defined(__CC_ARM))
    uint32_t ret = __builtin_popcount(a);
    sofie_restore_protected_stack();
    return ret;
#else
    a = a - ((a >> 1) & 0x55555555);
    a = (a & 0x33333333) + ((a >> 2) & 0x33333333);
    uint32_t ret = (((a + (a >> 4)) & 0xf0f0f0f) * 0x1010101) >> 24;
    sofie_restore_protected_stack();
    return ret;
#endif
}

// Find the sequence comparison of a and b, this is the distance
// between a and b ignoring overflow
static int lfs_scmp(uint32_t a, uint32_t b) {
	sofie_set_protected_stack();
	int ret = (int)(unsigned)(a - b);
	sofie_restore_protected_stack();
	return ret;
}

// Convert between 32-bit little-endian and native order
static uint32_t lfs_fromle32(uint32_t a) {
	sofie_set_protected_stack();
#if !defined(LFS_NO_INTRINSICS) && ( \
    (defined(  BYTE_ORDER  ) && defined(  ORDER_LITTLE_ENDIAN  ) &&   BYTE_ORDER   ==   ORDER_LITTLE_ENDIAN  ) || \
    (defined(__BYTE_ORDER  ) && defined(__ORDER_LITTLE_ENDIAN  ) && __BYTE_ORDER   == __ORDER_LITTLE_ENDIAN  ) || \
    (defined(__BYTE_ORDER__) && defined(__ORDER_LITTLE_ENDIAN__) && __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__))
	sofie_restore_protected_stack();
    return a;
#elif !defined(LFS_NO_INTRINSICS) && ( \
    (defined(  BYTE_ORDER  ) && defined(  ORDER_BIG_ENDIAN  ) &&   BYTE_ORDER   ==   ORDER_BIG_ENDIAN  ) || \
    (defined(__BYTE_ORDER  ) && defined(__ORDER_BIG_ENDIAN  ) && __BYTE_ORDER   == __ORDER_BIG_ENDIAN  ) || \
    (defined(__BYTE_ORDER__) && defined(__ORDER_BIG_ENDIAN__) && __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__))
    uint32_t ret = __builtin_bswap32(a);
    sofie_restore_protected_stack();
    return ret;
#else
    uint32_t ret = (((uint8_t*)&a)[0] <<  0) |
           (((uint8_t*)&a)[1] <<  8) |
           (((uint8_t*)&a)[2] << 16) |
           (((uint8_t*)&a)[3] << 24);
    sofie_restore_protected_stack();
    return ret;
#endif
}

static uint32_t lfs_tole32(uint32_t a) {
	sofie_set_protected_stack();
	uint32_t ret = lfs_fromle32(a);
	sofie_restore_protected_stack();
	return ret;
}

// Convert between 32-bit big-endian and native order
static uint32_t lfs_frombe32(uint32_t a) {
	sofie_set_protected_stack();
#if !defined(LFS_NO_INTRINSICS) && ( \
    (defined(  BYTE_ORDER  ) && defined(  ORDER_LITTLE_ENDIAN  ) &&   BYTE_ORDER   ==   ORDER_LITTLE_ENDIAN  ) || \
    (defined(__BYTE_ORDER  ) && defined(__ORDER_LITTLE_ENDIAN  ) && __BYTE_ORDER   == __ORDER_LITTLE_ENDIAN  ) || \
    (defined(__BYTE_ORDER__) && defined(__ORDER_LITTLE_ENDIAN__) && __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__))
	uint32_t ret = __builtin_bswap32(a);
	sofie_restore_protected_stack();
	return ret;
#elif !defined(LFS_NO_INTRINSICS) && ( \
    (defined(  BYTE_ORDER  ) && defined(  ORDER_BIG_ENDIAN  ) &&   BYTE_ORDER   ==   ORDER_BIG_ENDIAN  ) || \
    (defined(__BYTE_ORDER  ) && defined(__ORDER_BIG_ENDIAN  ) && __BYTE_ORDER   == __ORDER_BIG_ENDIAN  ) || \
    (defined(__BYTE_ORDER__) && defined(__ORDER_BIG_ENDIAN__) && __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__))
	sofie_restore_protected_stack();
    return a;
#else
    uint32_t ret = (((uint8_t*)&a)[0] << 24) |
           (((uint8_t*)&a)[1] << 16) |
           (((uint8_t*)&a)[2] <<  8) |
           (((uint8_t*)&a)[3] <<  0);
    sofie_restore_protected_stack();
    return ret;
#endif
}

static uint32_t lfs_tobe32(uint32_t a) {
	sofie_set_protected_stack();
	uint32_t ret = lfs_frombe32(a);
	sofie_restore_protected_stack();
	return ret;
}

// Calculate CRC-32 with polynomial = 0x04c11db7
uint32_t lfs_crc(uint32_t crc, const void *buffer, size_t size);

// Allocate memory, only used if buffers are not provided to littlefs
// Note, memory must be 64-bit aligned
static inline void *lfs_malloc(size_t size) {
#ifndef LFS_NO_MALLOC
    return umalloc(size);
#else
    (void)size;
    return NULL;
#endif
}

// Deallocate memory, only used if buffers are not provided to littlefs
static inline void lfs_free(void *p) {
#ifndef LFS_NO_MALLOC
    ufree(p);
#else
    (void)p;
#endif
}


#ifdef __cplusplus
} /* extern "C" */
#endif

#endif
#endif
