...
static void lfs_cache_zero(lfs_t *lfs, lfs_cache_t *pcache) {
    ...
    umemset(pcache->buffer, 0xff, lfs->cfg->cache_size);
...
static bool lfs_pair_isnull(const lfs_block_t pair[2]) {
	sofie_set_protected_stack();
	bool ret = pair[0] == LFS_BLOCK_NULL || pair[1] == LFS_BLOCK_NULL;
	sofie_restore_protected_stack();
	return ret;
}

static int lfs_pair_cmp(
        const lfs_block_t paira[2],
        const lfs_block_t pairb[2]) {
	sofie_set_protected_stack();
	int ret = !(paira[0] == pairb[0] || paira[1] == pairb[1] ||
...
#ifndef LFS_READONLY
static bool lfs_pair_sync(
        const lfs_block_t paira[2],
        const lfs_block_t pairb[2]) {
	sofie_set_protected_stack();
	bool ret = (paira[0] == pairb[0] && paira[1] == pairb[1]) ||
           (paira[0] == pairb[1] && paira[1] == pairb[0]);
	sofie_restore_protected_stack();
	return ret;
}
...
static uint16_t lfs_tag_type1(lfs_tag_t tag) {
	sofie_set_protected_stack();
    uint16_t ret = (tag & 0x70000000) >> 20;
    sofie_restore_protected_stack();
    return ret;
}
...
static uint16_t lfs_tag_type3(lfs_tag_t tag) {
	sofie_set_protected_stack();
    uint16_t ret = (tag & 0x7ff00000) >> 20;
    sofie_restore_protected_stack();
    return ret;
}
...
static int8_t lfs_tag_splice(lfs_tag_t tag) {
	sofie_set_protected_stack();
    int8_t ret = (int8_t)lfs_tag_chunk(tag);
    sofie_restore_protected_stack();
    return ret;
}
...
static uint16_t lfs_tag_id(lfs_tag_t tag) {
	sofie_set_protected_stack();
    uint16_t ret = (tag & 0x000ffc00) >> 10;
    sofie_restore_protected_stack();
    return ret;
}
...
static lfs_size_t lfs_tag_dsize(lfs_tag_t tag) {
	sofie_set_protected_stack();
	lfs_size_t ret = sizeof(tag) + lfs_tag_size(tag + lfs_tag_isdelete(tag));
	sofie_restore_protected_stack();
	return ret;
}
...
static uint8_t lfs_gstate_getorphans(const lfs_gstate_t *a) {
	sofie_set_protected_stack();
	uint8_t ret = lfs_tag_size(a->tag);
	sofie_restore_protected_stack();
	return ret;
}
...
static bool lfs_gstate_hasmovehere(const lfs_gstate_t *a,
	...
	sofie_set_protected_stack();
    bool ret = lfs_tag_type1(a->tag) && lfs_pair_cmp(a->pair, pair) == 0;
    sofie_restore_protected_stack();
    return ret;
}
...
static int lfs_alloc(lfs_t *lfs, lfs_block_t *block) {
    ...
    while (true) {
        ...
        umemset(lfs->free.buffer, 0, lfs->cfg->lookahead_size);
...
static lfs_stag_t lfs_dir_getslice(lfs_t *lfs, const lfs_mdir_t *dir,
    ...
    while (off >= sizeof(lfs_tag_t) + lfs_tag_dsize(ntag)) {
        ...
        if ((gmask & tag) == (gmask & (gtag - gdiff))) {
            ...
            umemset((uint8_t*)gbuffer + diff, 0, gsize - diff);
            sofie_restore_protected_stack();
            return tag + gdiff;
        }
    }
...
static lfs_stag_t lfs_dir_get(lfs_t *lfs, const lfs_mdir_t *dir,
	...
	sofie_set_protected_stack();
	lfs_stag_t ret = lfs_dir_getslice(lfs, dir,
            gmask, gtag,
...
static int lfs_dir_fetch(lfs_t *lfs,
    ...
    int ret = (int)lfs_dir_fetchmatch(lfs, dir, pair,
...
static int lfs_dir_commit_commit(void *p, lfs_tag_t tag, const void *buffer) {
    ...
    int ret = lfs_dir_commitattr(commit->lfs, commit->commit, tag, buffer);
    sofie_restore_protected_stack();
    return ret;
}
#endif
...
static int lfs_rawstat(lfs_t *lfs, const char *path, struct lfs_info *info) {
    ...
    int ret = lfs_dir_getinfo(lfs, &cwd, lfs_tag_id(tag), info);
    sofie_restore_protected_stack();
    return ret;
}
...
static int lfs_commitattr(lfs_t *lfs, const char *path,
    ...
    int ret = lfs_dir_commit(lfs, &cwd, LFS_MKATTRS(
...
static int lfs_rawsetattr(lfs_t *lfs, const char *path,
    ...
    int ret = lfs_commitattr(lfs, path, type, buffer, size);
    sofie_restore_protected_stack();
    return ret;
}
#endif
...
static int lfs_rawremoveattr(lfs_t *lfs, const char *path, uint8_t type) {
	sofie_set_protected_stack();
	int ret = lfs_commitattr(lfs, path, type, NULL, 0x3ff);
	sofie_restore_protected_stack();
    return ret;
}
#endif
...
static int lfs_fs_parent_match(void *data,
    ...
    int ret = (lfs_pair_cmp(child, find->pair) == 0) ? LFS_CMP_EQ : LFS_CMP_LT;
    sofie_restore_protected_stack();
    return ret;
}
#endif
...
static int lfs1_bd_read(lfs_t *lfs, lfs_block_t block,
	...
	int ret = lfs_bd_read(lfs, &lfs->pcache, &lfs->rcache, size,
...
///// Metadata pair and directory operations ///
static lfs_size_t lfs1_entry_size(const lfs1_entry_t *entry) {
	sofie_set_protected_stack();
	lfs_size_t ret = 4 + entry->d.elen + entry->d.alen + entry->d.nlen;
	sofie_restore_protected_stack();
	return ret;
}
